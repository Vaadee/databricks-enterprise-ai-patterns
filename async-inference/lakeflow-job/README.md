...
Async inference on Databricks using **Lakeflow serverless worker jobs**. Clients submit a document and get back a `job_id` immediately — no waiting for the model. A serverless Lakeflow Job picks up the work, streams the response to Lakebase (Postgres), and clients poll for status and results.
...
```bash
cd databricks-enterprise-ai-patterns/async-inference/lakeflow-job
```
...
```bash
databricks secrets create-scope lakeflow-job --profile $PROFILE
```

```bash
databricks secrets put-secret lakeflow-job azure-openai-endpoint \
  --string-value "https://<your-resource>.openai.azure.com/" \
  --profile $PROFILE

databricks secrets put-secret lakeflow-job azure-openai-key \
  --string-value "<your-key>" \
  --profile $PROFILE

databricks secrets put-secret lakeflow-job azure-deployment-name \
...
Expected output:
```
Connecting to project=lakeflow-job branch=production db=databricks_postgres
Schema migration complete.
Created Postgres role and granted table access to app SP <uuid> (lakeflow-job-dev)
```
...
```bash
databricks bundle run lakeflow_job_app --profile $PROFILE
```
...
```bash
databricks apps get lakeflow-job-dev --profile $PROFILE --output json \
  | python3 -c "import sys,json; print(json.load(sys.stdin)['url'])"
```

Set it as a variable for the curl commands below:
```bash
export APP_URL=https://lakeflow-job-dev-<workspace-id>.databricksapps.com
```
...
databricks bundle run lakeflow_job_app --target acme-prod
```
...
databricks bundle run lakeflow_job_app --target prod --profile $PROFILE
```
...
|---|---|---|
| `app_name` | `lakeflow-job` | Base name for all resources |
| `secret_scope` | `lakeflow-job` | Secret scope for Azure OpenAI credentials |
| `service_principal` | `lakeflow-job-sp` | SP name for prod `run_as` |
| `lakebase_project_id` | `lakeflow-job` | Lakebase Autoscaling project ID |
...
### App logs
```bash
databricks apps logs lakeflow-job-dev --profile $PROFILE
```

For specific errors:
```bash
databricks apps logs lakeflow-job-dev --profile $PROFILE 2>&1 | grep -i "error\|failed\|exception" | tail -20
```
...
- Worker runs: **Jobs → `[dev] lakeflow-job-worker`**
- Reconciler runs: **Jobs → `[dev] lakeflow-job-reconciler`**

### Check permissions on the Lakebase project
```bash
databricks api get /api/2.0/permissions/database-projects/lakeflow-job \
...
```bash
APP_URL=https://lakeflow-job-dev-<workspace-id>.databricksapps.com \
...
| Quirk | Explanation |
|---|---|
| `bundle deploy` doesn't restart the app | Always follow with `bundle run lakeflow_job_app`. Databricks Apps requires an explicit new deployment to pick up code changes. |
