#!/usr/bin/env bash
# start_demo.sh — Deploy and start the lakeflow-job demo
#
...
cat << 'EOF'
  ╔══════════════════════════════════════════════════════════╗
  ║   lakeflow-job  —  demo start                           ║
  ║   Async LLM inference on Databricks Apps + Lakebase     ║
  ╚══════════════════════════════════════════════════════════╝
EOF
printf "${NC}\n"
printf "  This script will:\n"
printf "  ${DIM}1.${NC} Deploy the Databricks Asset Bundle (app + worker + reconciler + Lakebase + MLflow)\n"
...
info "Deploying: app, Lakeflow worker job, reconciler job, Lakebase project, MLflow experiment, schema migration job"
echo ""
...
if ! databricks bundle run lakeflow_job_app --profile "$PROFILE" 2>&1 | \
    while IFS= read -r line; do printf "  ${DIM}│${NC} %s\n" "$line"; done; then
...
printf "  ${BOLD}Note:${NC} The reconciler job runs every 5 min automatically — no action needed.\n\n"
