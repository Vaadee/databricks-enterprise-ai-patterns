...
def stream_completion(conn: psycopg.Connection, job_id: str, payload: dict) -> None:  # noqa: C901
    ...
    try:
        ...
        try:
            ...
            if (
                is_foundation_model
                and "guardrail" in str(guardrail_exc).lower()
                and "streaming" in str(guardrail_exc).lower()
            ):
                logger.warning(
                    "job_id=%s workspace output guardrails block streaming — retrying without stream",  # noqa: E501
                    job_id,
                )
        ...
        for event in stream:
            ...
            if len(buffer) >= FLUSH_EVERY:
                ...
                logger.debug(
                    "job_id=%s chunk=%d flushed (%d chars)", job_id, chunk_index, len(buffer)
                )
                chunk_index += 1
                buffer = ""
