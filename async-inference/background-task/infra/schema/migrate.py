...
def main():
    ...
    project_id = args.lakebase_project_id or os.environ.get(
        "LAKEBASE_PROJECT_ID", "background-task"
    )
        ...
        try:
            ...
            with get_connection(w, project_id, branch_id, db_name) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        pgsql.SQL("GRANT USAGE ON SCHEMA public TO {}").format(
                            pgsql.Identifier(client_id)
                        )
                    )
                    cur.execute(
                        pgsql.SQL(
                            "GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO {}"  # noqa: E501
            ...
            logger.info(
                "Created Postgres role and granted table access to app SP %s (%s)",
                client_id,
                app_name,
            )
    ...
    if mlflow_experiment_name and client_id:
        try:
            ...
            if exp_id:
                ...
                logger.info(
                    "Granted CAN_MANAGE on MLflow experiment %s to app SP %s",
