#!/usr/bin/env python3
"""
smoke_test.py — End-to-end smoke test for the background-task app.
...
Usage:
    ...
    export APP_URL=https://background-task-dev-<workspace>.databricksapps.com
...
def get_token() -> str:
    ...
    return json.loads(_run_cli("auth", "token", "--profile", PROFILE, "--output", "json"))[
        "access_token"
    ]
...
def run_tests():
    if not APP_URL:
        ...
        print("  export APP_URL=https://background-task-dev-<workspace>.databricksapps.com")
        sys.exit(1)

    token = get_token()

    print(f"\n{bold('╔══════════════════════════════════════════════════╗')}")
    print(f"{bold('║   background-task  smoke test         ║')}")
    print(f"{bold('╚══════════════════════════════════════════════════╝')}")
    print(f"\n  {dim('APP_URL:')} {APP_URL}")
    ...
    check("GET /openapi.json → 200", code == 200, f"paths={len(body.get('paths', {}))}")
    ...
    check("GET /jobs/status/<random-uuid> → 404", code == 404, body.get("detail", ""))

    code, body = GET(f"/jobs/result/{bogus}", token)
    check("GET /jobs/result/<random-uuid> → 404", code == 404, body.get("detail", ""))
    ...
    if code != 202:
        ...
        check("Initial status is PENDING", body.get("status") == "PENDING", body.get("status"))
    ...
    if code != 202:
        ...
        check("Long job reached DONE", final_status == "DONE", f"status={final_status}")
        # RUNNING is often missed — job moves PENDING→RUNNING→STREAMING in < 2s
        if saw_running:
            ok("Observed RUNNING state")
        else:
            warn(
                "Did not observe RUNNING state (transitions faster than poll interval — not a failure)"  # noqa: E501
            )
        if saw_streaming:
        ...
        if final_status == "DONE":
            ...
            check(
                "Long job result has text",
                bool(result.get("result")),
                f"{len(result.get('result', ''))} chars",
            )
            check(
                "Long job total_tokens > 0",
                (result.get("total_tokens") or 0) > 0,
                str(result.get("total_tokens")),
            )
    ...
    check("Both submits → 202", all_202, " ".join(str(c) for c, _ in codes_and_ids))
    check("Both have distinct job_ids", unique_ids, " ".join(str(jid) for _, jid in codes_and_ids))
