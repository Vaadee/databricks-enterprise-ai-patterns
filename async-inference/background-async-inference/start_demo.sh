#!/usr/bin/env bash
# start_demo.sh — Deploy and start the background-async-inference demo
#
...
cat << 'EOF'
  ╔══════════════════════════════════════════════════════════╗
  ║   background-async-inference  —  demo start             ║
...
if ! databricks bundle run background_async_inference_app --profile "$PROFILE" 2>&1 | \
    while IFS= read -r line; do printf "  ${DIM}│${NC} %s\n" "$line"; done; then
