...
class LakebaseConnectionManager:
        ...
        while True:
            ...
            try:
                self._token = await asyncio.to_thread(
                    _generate_token, self._w, self._ep_name
                )
                ...
                if self._token_refresh_failures >= 3:
                    logger.error(
                        ...
                        "Check Databricks workspace connectivity and service principal credentials.",
    ...
    def get_sync_conn(self) -> psycopg.Connection:
        """
        Create a fresh synchronous connection using the current OAuth token.

        Call this from inside a thread pool thread (asyncio.to_thread) — NOT from
        the event loop — so the connection is created and used in the same thread,
        avoiding psycopg3 thread-crossing violations.

        self._token is a plain string; reads from any thread are safe under the GIL.
