#!/usr/bin/env bash
# stop_demo.sh — Stop (and optionally destroy) the background-async-inference demo
#
# By default: stops the app and leaves all resources in place (safe, fast).
# With --destroy: also runs `bundle destroy` to remove the app, Lakebase
#                 project, MLflow experiment, and schema migration job.
#
# Usage:
#   bash stop_demo.sh
...
cat << 'EOF'
  ╔══════════════════════════════════════════════════════════╗
  ║   background-async-inference  —  demo stop              ║
  ╚══════════════════════════════════════════════════════════╝
EOF
printf "${NC}\n"

if [ "$DO_DESTROY" = "1" ]; then
  printf "  ${R}${BOLD}Destroy mode:${NC} This will remove the app, Lakebase project,\n"
  printf "  MLflow experiment, and schema migration job.\n"
  printf "  ${Y}Postgres data (job_requests / job_chunks / job_results) will be lost.${NC}\n"
else
  printf "  ${DIM}Stopping the app. All resources (Lakebase, MLflow, jobs) are kept.\n"
...
echo ""
prompt APP_NAME "App name" "background-async-inference"
